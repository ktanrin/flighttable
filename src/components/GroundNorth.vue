...
export default {
  ...
  computed: {
    standsToInclude() {
      // Define the stands you want to include here
      const specificStands = [
      ...
      return this.flights.filter((flight) => specificStands.includes(flight.DepartureGate));
    },
    filteredFlights() {
      //return this.standsToInclude;
       const now = new Date();
       return this.standsToInclude.filter((flight) => {
        if (flight.CTOTSTR !== '') {
